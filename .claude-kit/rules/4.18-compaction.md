...
> Dipindah dari kernel lama §4.18 (`CLAUDE_universal_v1`, kini diarsip; kernel = `AGENTS.md`). Tak ada stub always-load — jalan masuk = pemicu di `rules/INDEX.md`. Detail langkah + larangan + contoh di sini (dibaca saat dipakai saja, hemat token always-load).
...
**Yang BUKAN sasaran:** kode (logika), aturan inti (`AGENTS.md`), berkas config/secret. Compaction = rapi-rapi **ringkasan/index**, BUKAN refactor kode (itu Mode Refactor Bertingkat — `rules/4.2-pattern-driven.md`).
...
1. **Tentukan sasaran.** Pakai sinyal nyata: berkas index lewat batas muat? daftar-isi tak sinkron dengan berkasnya? `Grep`/robot, bukan tebakan. Cakupan ke yang bengkak/melenceng saja (§6.3 — jangan over-deploy).
   - **Kalau sasarannya berkas ATURAN** (`AGENTS.md`): jalankan dulu `npx lintasai locked-phrases` (default sudah `AGENTS.md`; `--file <berkas>` untuk berkas aturan lain). Ia mencetak frasa harfiah yang DIKUNCI tes — teks yang kelihatan seperti basa-basi tapi kalau hilang bikin tes merah. Terjadi nyata 2026-07-18: frasa `Pengecualian 8 skill divisi WAJIB` (§4.9) ikut terpangkas, baru ketahuan setelah tes merah.
