...
- `AGENTS.md` (kernel akar) — judul `> Versi X.Y.Z · ...` (kalau aturan ikut berubah)
...
- Penomoran: ikut semver kit — perbaikan kecil = angka KECIL, fitur = MENENGAH,
  breaking = BESAR. Label `[SECURITY]` untuk perbaikan keamanan mendesak.

### 2. Tambah / ubah ATURAN di `AGENTS.md` (kernel akar, auto-baca tiap sesi staff; detail on-demand → rak `rules/`)
...
- `AGENTS.md` — tulis aturan + naikkan versi judul
...
- Perujuknya (`AGENTS.md` dll.) — tulis rujukan sebagai **path** `rules/<berkas>.md`;
...
- **Khusus SKILL DIVISI (§4.13):** nama divisi juga hidup di hub `rules/4.13-division-skills.md` +
  `rules/INDEX.md` dan blob governor `tests/skills-divisi.test.mjs`. Tambah divisi baru = perbarui SEMUA lokasi itu.
- ➡️ **Dikunci MESIN:** `tests/roster-sync.test.mjs` (divisi kanonik — kini Backend + SEO — konsisten hub
  `rules/4.13-division-skills.md` + `rules/INDEX.md`, + 10 lensa §4.1 + ambang MATURE) +
  `tests/skills-divisi.test.mjs` (isi checklist) + `node engine/rules-ref-check.mjs`.
...
- **SEBUT DI ATURAN** — kalau tak disebut `AGENTS.md`/rak `rules/`-nya, AI tak akan pernah tahu ia ada; itu
