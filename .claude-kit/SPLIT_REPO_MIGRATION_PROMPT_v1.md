...
- Per folder, smoke check (checklist lengkap di `SPLIT_REPO_PREPROVISION_v1.md` > "Validasi per folder"):
  ...
  - **lintasAI terpasang**: `AGENTS.md` akar (kernel, placeholder terisi) + `CLAUDE.md` loader + folder `.claude-kit/` ada
...
> Catatan internal AI (JANGAN ikut ditampilkan ke user): per RULE-4b (`rules/14.1-popup-ui.md`) — opsi yang disarankan WAJIB di posisi [1] supaya staff yang tekan Enter langsung ikut rekomendasi. Detail penjelasan tiap opsi ada di dalam blok di bawah.
