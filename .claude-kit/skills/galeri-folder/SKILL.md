...
3. 📐 **Stack di luar dua galeri §3 (bukan Next.js/React atau Python FastAPI/Django) → ikuti konvensi resmi stack itu sendiri**, tetap penuhi standar inti aturan kerja (§5 standar kode, §8 keamanan minimum, §9 DB & data → `skills/database/SKILL.md`, §10 frontend/UX/SEO, §11 proses → `skills/devops/SKILL.md`; peta bidang: `rules/4.13-division-skills.md`).
...
- 👨‍💻 Programmer: validasi input di boundary (`app/api/*`, server action) pakai schema (mis. `zod`); tipe lintas-modul didefinisikan sekali di `types/` lalu di-import — jangan ditebak inline (selaras §5 standar kode). Detail lengkap Next.js → `skills/next-core/SKILL.md`.
...
- 📐 **Detail teknis Next.js/React** (versi terpasang, pola lanjut) → `skills/next-core/SKILL.md` · `skills/react-patterns/SKILL.md`.
