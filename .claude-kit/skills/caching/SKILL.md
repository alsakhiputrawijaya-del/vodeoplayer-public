...
divisi: backend
pemicu: [cache, caching, cached, cache-aside, stale-while-revalidate, cache-invalidation, invalidasi-cache, thundering-herd, cache-stampede, memcached, simpan-sementara]
...
> **Kapan skill ini aktif:** prompt menyentuh "cache / simpan-sementara / bikin cepat / hasil query berat disimpan / stale-while-revalidate / invalidasi cache / halaman berat". Dispatcher `rak-pemicu` menyalakannya otomatis.
>
> 🙂 **Analogi:** cache = **catatan tempel di meja kasir**. Daripada bolak-balik ke gudang (database) tiap ditanya harga, kasir menempel harga yang sering ditanya di meja — jawaban instan. Bahayanya: kalau harga di gudang berubah tapi catatan tempel tak diperbarui, pelanggan diberi harga **basi**. Maka aturan mainnya: catatan tempel harus punya **masa berlaku** (TTL) + **dicabut saat harga berubah** (invalidasi).

Skill ini **advisory** (§4.17): otak native memutuskan adopsi/adaptasi/abaikan tiap 📐/💡. Butir **🔒 HASIL** = jaminan yang tak boleh gagal (di sini: cache TAK BOLEH membocorkan data antar-user & TAK BOLEH jadi satu-satunya sumber kebenaran). Cek dokumentasi cache/framework **versi terpasang** (§8.2 A3) — API TTL/revalidate/ISR beda antar-sistem/versi.
...
- 🔒 **HASIL:**
  ...
  - **Output — miss/expired:** hitung dari sumber → simpan → kembalikan. Sistem **tetap benar walau cache kosong** (cache = optimasi, BUKAN storage/sumber kebenaran).
...
6. 📐 **Cache berlapis, makin dekat user makin murah:** browser (`Cache-Control`/`ETag`) → CDN/edge (`s-maxage`, SWR) → aplikasi (Redis/memori) → query DB. Halaman publik statis → CDN + ISR/revalidate (Next.js) → `skills/next-core/SKILL.md` · `skills/cloudflare/SKILL.md`. Data lintas-instance → **Redis/memcached** (memori-proses tak terbagi antar-server, hit-rate anjlok di multi-instance).
...
- 💡 SARAN: pakai pustaka/fitur teruji ketimbang menulis sendiri — Next.js `revalidate`/`unstable_cache`, `@upstash/redis`, HTTP `Cache-Control: stale-while-revalidate`. Cek API **versi terpasang** (§8.2 A3).
...
- [ ] **Hit-rate dipantau**; eviction (LRU) di-set; cache bukan sumber kebenaran?
...
- 📐 Cache HTTP/ISR/revalidate + performa render → `skills/next-core/SKILL.md`; cache edge/CDN → `skills/cloudflare/SKILL.md`.
