...
divisi: frontend
pemicu: [aksesibilitas, a11y, wcag, disabilitas, difabel, tunanetra, buta-warna, pembaca-layar, screen-reader, kontras, aria, focus-trap]
...
> **Kapan skill ini aktif:** prompt menyentuh "aksesibilitas / a11y / WCAG / bisa dipakai difabel / tunanetra / pembaca layar / kontras / buta warna / navigasi keyboard / microcopy". Otomatis relevan TIAP bikin `<input>`, modal, dropdown, tombol-ikon, atau animasi. Dispatcher `rak-pemicu` menyalakannya otomatis (staff tak perlu mengetik nama skill).
>
> 🙂 **Analogi:** aksesibilitas = **ramp + pegangan tangga + papan petunjuk jelas** di gedung. Bukan cuma orang berjalan normal yang bisa masuk — pengguna kursi roda, lansia, tunanetra juga. Membangunnya = memasang jalur itu SEJAK AWAL, bukan menambal setelah ada yang komplain tak bisa masuk.

Skill ini **advisory** (§4.17): otak native memutuskan adopsi/adaptasi/abaikan tiap butir 📐/💡. Tapi butir **🔒 HASIL** = hasil yang tak boleh gagal apa pun caranya (di sini: orang dengan keterbatasan tetap bisa memakai halaman). Angka ambang wajib (kontras, target sentuh) ada di §1b aturan — **jangan dikarang** (§8.2 Aturan 1). *(a11y = singkatan "accessibility": "a" + 11 huruf + "y". WCAG = Web Content Accessibility Guidelines, standar aksesibilitas resmi W3C. ARIA = label tak-terlihat yang memberi tahu pembaca layar "ini tombol/menu/dialog".)*
...
> Terapkan OTOMATIS tiap menulis/menilai `<input>`, modal, dropdown, tombol-ikon, atau animasi — bukan tambahan opsional. Tiap pola diberi label keseriusan: 📐 = PENTING (keamanan pengguna disabilitas + hukum aksesibilitas) · 💡 = RAPIKAN. Ambil polanya, **jangan salin mentah** — sesuaikan ke framework/versi terpasang (§8.2 A3).
...
```
🙂 tiap kotak isian punya "nama tertulis" yang nyambung — kayak label di loker.
...
**3. Pakai elemen HTML yang tepat (semantik), jangan `<div onClick>`.** 📐 PENTING. Tombol → `<button type="button">` (bisa fokus + Enter/Space + diumumkan "button"); navigasi → `<a href>`. `<div onClick>` butuh `role`+`tabIndex={0}`+`onKeyDown` manual (gampang lupa). 🙂 pakai "tombol asli", bukan "kotak dicat mirip tombol".
...
> **Verifikasi WAJIB cuma-baca** (§8.2 Aturan 3): membuktikan = baca kode/markup + uji keyboard + `Grep`, JANGAN jalankan perintah yang mengubah data live.
...
- 📐 **Angka ambang** (kontras 4,5:1, non-teks 3:1, target sentuh 24px, Core Web Vitals) → §1b aturan (sumber angka; jangan dikarang).
