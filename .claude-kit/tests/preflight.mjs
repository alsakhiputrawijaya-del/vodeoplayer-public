...
// Penjaga anggaran token berkas aturan always-load (CLIENT.md). Cegah bloat berulang yang
...
function checkRulesBudget(repoRoot) {
  try {
    ...
    if (!res.present) return { id: 'rules-budget', label: 'Anggaran token berkas aturan', level: LEVEL.INFO, detail: 'AGENTS.md tak ketemu (root / .claude-kit/) - dilewati.' }
...
// Penjaga anggaran token GABUNGAN (ADR-019): AGENTS.md + AGENTS.override.md + CLAUDE.md - SEMUA
...
function checkAlwaysLoadBudget(repoRoot) {
  try {
    ...
    if (!res.present) return { id: 'rules-budget-total', label: 'Anggaran token gabungan (always-load)', level: LEVEL.INFO, detail: 'AGENTS.md tak ketemu (root / .claude-kit/) - dilewati.' }
    const budgetTok = Math.round(res.budgetChars / CHARS_PER_TOKEN)
