...
|---|---|---|
| **Aturan lengkap** (bahasa non-programmer, standar profesional, anti-ngarang, gerbang QA, dll) | ✅ Otomatis | `npx lintasai init`/`update` memasang kernel `AGENTS.md` di akar project. Kimi Code membacanya **native tiap sesi** (standar `AGENTS.md`) — kualitas **sama seperti Claude**; tidak ada berkas aturan khusus Kimi. |
...
> (b) aturan anti-injection + anti-halusinasi kit (rak `rules/module/8.1-anti-injection.md` + `rules/8.2-anti-hallucination.md`) yang aktif lewat `AGENTS.md` akar.
...
Ini otomatis memasang/memperbarui kernel `AGENTS.md` di akar project. Buka project di Kimi Code →
aturan langsung aktif (Kimi membaca `AGENTS.md` akar secara native; **tidak ada lagi** berkas
`.kimi-code/AGENTS.md` generated). (Perintah manual bila perlu: `npx lintasai adapter-sync --write` —
regenerasi adapter Cursor; Kimi & Codex membaca `AGENTS.md` akar langsung.)
...
**a2) Aturan terbaca UTUH (bukan terpotong)?** Ketik: `sebutkan 5 bagian kernel aturan kerja tim ini`.
→ Jawaban harusnya menyebut **Bahasa · Anti-halusinasi · Struktur-rapi · Loop kerja · AI tidak boleh
   merusak** — bagian 5 ada di BAGIAN AKHIR `AGENTS.md`. Kalau tersebut = kernel (ramping, ~8 KB)
   terbaca **penuh**. Kalau Kimi tak tahu → mungkin berkas terpotong; lapor ke tim (jaring pengaman;
   dokumen resmi Kimi menyatakan **tak ada** batas ukuran, jadi seharusnya aman).
...
Kalau (a) gagal (jawaban Inggris / tak terasa mengikuti aturan): pastikan `AGENTS.md` ada di akar
folder project (jalankan `npx lintasai init`/`npx lintasai@latest update` kalau belum), lalu buka sesi Kimi BARU.
...
- `AGENTS.md` akar project = **kernel aturan** yang dibaca Kimi native (dipasang/diperbarui
  `npx lintasai init`/`update`). Tidak ada lagi `.kimi-code/AGENTS.md` generated — isi `.kimi-code/`
  tinggal `config.toml` (hook opt-in).
