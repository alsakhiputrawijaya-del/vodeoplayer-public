...
> Versi 3.1.0 · 2026-07-23 · Microkernel universal lintas-stack & lintas-harness.
...
> Robot/hook = penguat opsional (Claude penuh · Kimi sebagian · Codex/Cursor tak ada) → SEMUA aturan jalan dari penalaran.
...
> PATH: kit di-embed di `.claude-kit/` → semua `skills/...` & `registry.json` di berkas ini ADA di `.claude-kit/skills/...` (working dir AI = root project).
...
1.6  Jawaban ringkas: langsung ke inti, tanpa basa-basi/pengulangan (hemat token + client tak kewalahan).
     Ringkas ≠ tanpa penjelasan — KENAPA/TUJUAN (1.3) tetap ada.
...
3.5  Konvensi rinci per-stack → ikut skill stack yang dibuka di langkah 4.3 (next-core · supabase-prisma ·
     galeri-folder). Prinsip umum sudah di 3.1–3.4.
...
4.2  DETEKSI (semantik, bukan cocok-kata) — simpulkan bidang walau tanpa kata teknis: "biar orang bisa
     ...
     Fitur web biasanya lintas-divisi. Petakan ke skill via skills/registry.json; tak ada yang persis →
     pilih terdekat, selalu arahkan ke ≥1 skill.
...
4.5  PUTUSKAN (sebagai <divisi>-profesional dari 4.2 DETEKSI) — pakai teknik & standar dari SKILL (4.3),
     ...
     perlu · tinjauan multi-divisi tampil hanya bila ada temuan nyata.
