...
//
// KENAPA ADA: `AGENTS.md` dibaca AI **tiap sesi** (biaya token TETAP per-sesi, di kit
...
// Anggaran GABUNGAN (AGENTS.md + AGENTS.override.md + CLAUDE.md) - ADR-019. Sebelumnya robot ini
// cuma mengukur 1 berkas kernel sendirian, padahal client dapat DUA berkas @import lain
...
// Cari berkas always-load (kernel AGENTS.md): kit (root) ATAU client (root/.claude-kit fallback).
// Sejak ADR-032 kernel = AGENTS.md akar. Return path pertama yang ada, atau null.
export function findAlwaysLoadFile(repoRoot = process.cwd()) {
  const candidates = [
    path.join(repoRoot, 'AGENTS.md'),
    path.join(repoRoot, '.claude-kit', 'AGENTS.md'),
  ]
...
// Cari berkas TEMAN yang sama-sama @import tiap sesi (AGENTS.override.md, CLAUDE.md) relatif ke anchor
// (kernel AGENTS.md). Anchor di "<root>/AGENTS.md" -> companion root = dir yang sama. Fallback: kalau
// anchor kebetulan di ".claude-kit/", naik 1 level. Return null per-berkas kalau tak ada (auto-skip anggun).
export function findCompanionAlwaysLoadFiles(anchorPath) {
  if (!anchorPath) return { overrideMdPath: null, claudeMdPath: null }
  ...
  const overrideMdPath = path.join(companionRoot, 'AGENTS.override.md')
  const claudeMdPath = path.join(companionRoot, 'CLAUDE.md')
  return {
    overrideMdPath: fs.existsSync(overrideMdPath) ? overrideMdPath : null,
...
// Orkestrasi BARU (ADR-019): total gabungan SEMUA berkas @import tiap sesi (AGENTS.md +
// AGENTS.override.md + CLAUDE.md kalau ada). BUKAN pengganti runRulesBudget() - itu tetap fokus 1 berkas (pesan
...
export function runAlwaysLoadBudget({ repoRoot = process.cwd(), budgetChars = DEFAULT_TOTAL_BUDGET_CHARS, filePath = null } = {}) {
  ...
  const { overrideMdPath, claudeMdPath } = findCompanionAlwaysLoadFiles(anchorPath)
  const dasar = [
    { role: 'AGENTS.md', path: anchorPath },
    ...(overrideMdPath ? [{ role: 'AGENTS.override.md', path: overrideMdPath }] : []),
  ...
  // lewat penelusuran ini. Dedup memakai path absolut, jadi rantai normal (CLAUDE.md -> AGENTS.md)
...
function main() {
  ...
  if (!res.present) {
    console.log('Anggaran token berkas aturan: DILEWATI - AGENTS.md tak ketemu (root / .claude-kit/).')
    process.exit(0)
  }
  ...
  // di atas (yang tetap fokus kernel AGENTS.md sendirian untuk pesan "pindah ke rules/").
