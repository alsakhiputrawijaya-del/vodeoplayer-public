#!/usr/bin/env node
// engine/agents-md.mjs - Deploy AGENTS.md + CLAUDE.md loader (port Node dari agents-md.ps1).
//
...
// Publish AGENTS.override.md ke project root: fill template + backup existing.
...
export function publishAgentsOverrideMd({ projectRoot, templatePath, placeholders = {}, preserve = true } = {}) {
  ...
  const target = path.join(projectRoot, 'AGENTS.override.md')
  const existing = isFile(target)
...
export function publishClaudeMd({ projectRoot, templatePath, placeholders = {} } = {}) {
  ...
  // `@./AGENTS.override.md`. Cek tunggal `@./AGENTS.md` salah-deteksi loader lama sebagai 'current' →
  ...
  if (existing) {
    ...
    if (cur && cur.includes('@./AGENTS.md') && cur.includes('@./AGENTS.override.md') && !cur.includes('@./.claude-kit/')) {
...
if (isMain) console.log('engine/agents-md.mjs - library (import publishAgentsMd / publishClaudeMd).')
