...
export function checkAiTools(kitDir, projectRoot, emit) {
  ...
  // Kimi Code - sejak ADR-032 membaca kernel AGENTS.md akar NATIVE (tak ada lagi salinan
  // .kimi-code/AGENTS.md generated). Yang khas-Kimi tersisa = hook opt-in (.kimi-code/config.toml).
  if (ada('AGENTS.md')) {
    if (ada('.kimi-code', 'config.toml')) {
      emit('OK    Kimi Code      : aturan = AGENTS.md akar (native) · hook terpasang -> palang BERLAKU')
    } else {
      emit('OK    Kimi Code      : aturan = AGENTS.md akar (native) · hook opt-in belum dipasang -> palang TIDAK berlaku (lihat KIMI_CODE_SETUP.md)')
    }
    c.ok++
  } else {
    emit('INFO  Kimi Code      : AGENTS.md akar tak ada - aturan tak termuat di Kimi (jalankan npx lintasai init / update).')
  }
  ...
  // Codex & Kimi - baca AGENTS.md akar NATIF (ADR-032): kernel = sumber tunggal, tak perlu generator.
  if (ada('AGENTS.md')) {
    emit('OK    Codex/Kimi     : baca kernel AGENTS.md akar secara native (sumber tunggal) · palang TIDAK berlaku')
    emit('        -> Codex & Kimi tak punya sistem hook (Kimi: hook OPT-IN terpisah). Yang sampai = teks aturannya.')
    c.ok++
  } else {
    emit('INFO  Codex/Kimi     : AGENTS.md akar tak ketemu - kernel belum terpasang di project ini.')
  }
