...
//
// SEJAK ADR-032: kernel = `AGENTS.md` akar project = SUMBER TUNGGAL, dibaca NATIVE oleh Codex, Kimi Code,
...
// kitPrefix untuk pointer rak = '.claude-kit/' kalau folder itu ada (client), '' kalau dogfood.
// Return {path, kitPrefix} atau null.
export function findRulesSource(repoRoot = process.cwd()) {
  const kitPrefix = fs.existsSync(path.join(repoRoot, '.claude-kit')) ? '.claude-kit/' : ''
  const atRoot = path.join(repoRoot, 'AGENTS.md')
  const atClient = path.join(repoRoot, '.claude-kit', 'AGENTS.md')
...
function main() {
  ...
  if (!cursor.present) {
    console.log('adapter-sync: DILEWATI - AGENTS.md tak ketemu (root / .claude-kit/).')
    process.exit(0)
  }
