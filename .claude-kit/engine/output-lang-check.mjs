#!/usr/bin/env node
// engine/output-lang-check.mjs - Robot penjaga bahasa: pastikan TULISAN kit ke staff tetap
// Bahasa Indonesia awam, bukan prosa Inggris. (ADR-004 #3 -
...
// Robot ini = jaring deterministik (~0 token, selaras doktrin §6.3 (rules/4.6-6.3-efficiency-doctrine.md)): scan string
...
// aturan bahasa AGENTS.md §1), ATAU (b) >=2 kata-fungsi Inggris bersinyal-kuat DAN TANPA penanda
...
// Frasa-penghubung khas AI-narasi Inggris (bocoran terdokumentasi aturan bahasa AGENTS.md §1).
const CONNECTORS = [
...
// Daftar bawaan = kode Node PRODUKSI yang menulis ke staff: engine/*.mjs + bin/lintasai.js +
// engine/risk-gate.js + orkestrator akar yang sudah diport (ROOT_ORCHESTRATORS). Inilah lingkup gerbang.
export function defaultNodeFiles(repoRoot) {
  ...
  try {
    for (const name of fs.readdirSync(engineDir)) {
      if (name.endsWith('.mjs') || name === 'risk-gate.js') files.push(path.join('engine', name))
    }
  } catch { /* engine/ tak ada -> lewati */ }
...
if (isMain) {
  // CLI: `node engine/output-lang-check.mjs [berkas...]` (tanpa argumen = pindai kode Node produksi).
