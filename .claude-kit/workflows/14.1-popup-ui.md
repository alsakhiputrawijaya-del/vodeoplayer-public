...
**Dynamic popup (project-context aware)**: popup yang option-nya berubah sesuai project state (mis. `JALANKAN_KIT.md` Popup #3, atau Quick Wins dari audit) → AI WAJIB auto-detect state (size/team/stack maturity/findings) lalu re-order supaya paling relevan di `[1]`. Hardcoded order = fallback kalau auto-detect gagal.
...
- **RULE-8 (Popup klik dulu, teks cadangan)**: Untuk popup Tipe A (pertanyaan AI ke user di chat), kalau harness menyediakan tool popup-pilihan native (di Claude Code: `AskUserQuestion` — kotak pilihan yang BISA DIKLIK), AI **WAJIB** render lewat tool itu. Sub-aturan:
  ...
  - **(f) Bahasa tampilan WAJIB awam**: saat merender popup (klik MAUPUN teks), AI WAJIB menerjemahkan jargon mentah di blok kanonik ke bahasa non-programmer (§2.1) — makna, jumlah, nomor opsi TIDAK berubah. Blok kanonik = sumber MAKNA, bukan izin menampilkan jargon apa adanya.
