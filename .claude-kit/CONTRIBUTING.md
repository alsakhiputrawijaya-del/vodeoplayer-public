...
1. **Naikkan versi: `node kit.mjs bump X.Y.Z`** (cap otomatis 6 berkas + kerangka CHANGELOG, lihat di atas) → tulis deskripsi CHANGELOG → jalankan `npm run preflight` — wajib semua lulus.
...
> ⚠️ **Kenapa wajib tunggu hijau (anti beda-versi):** penanda versi (git tag) langsung publik begitu di-push, dan jalur **update kit** (`npx lintasai update`) membaca versi terbaru dari **git tag** — sedangkan **pasang baru** (`npm create lintasai`) membaca dari **npm**, yang baru terisi SETELAH robot penerbit selesai. Kalau robot **gagal** (mis. `NPM_TOKEN` kedaluwarsa) tapi tag terlanjur dikirim, staff yang *update* dapat versi BARU sementara staff yang *pasang-baru* dapat versi LAMA = tim beda-versi. Jadi: kalau robot **merah**, perbaiki + jalankan ulang (*Run workflow* / `workflow_dispatch`) **SEBELUM** mengumumkan rilis ke staff.
