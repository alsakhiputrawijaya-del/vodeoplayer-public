...
5. **Bahasa tampilan WAJIB awam (per `AGENTS.md` §1)**: label opsi jargon mentah (mis. "monorepo", "split repo") WAJIB diterjemahkan saat merender — makna & nomor opsi TIDAK berubah. (Istilah programming boleh dipertahankan + gloss di deskripsi opsi — term-first `AGENTS.md` §1 + `rules/ref-jargon-card.md`.)
...
7. Baca `AGENTS.md` (kernel akar) + `AGENTS.override.md` (kalau ada) untuk load aturan.
...
Pakai mesin `rules/4.2-pattern-driven.md` ("Mode Refactor Bertingkat"). **Sebelum mengubah kode existing (pengaman "simpan dulu"):** AI cek `git status`. Kalau ada perubahan menggantung → popup: `[1] Simpan dulu kerjamu (commit) sebelum aku rapikan (rekomendasi) / [2] Lanjut saja (perubahan yang belum tersimpan akan ikut tercampur)`. JANGAN langsung ubah kode di atas perubahan belum tersimpan.
...
Setelah Bagian 1-2 selesai, kalau kamu **staff IT non-programmer baru pertama kali pakai lintasAI**, AI akan auto-trigger **Guided Step-by-Step Pattern** (`rules/4.3-guided-step-by-step.md`):
...
Detail lengkap di `rules/4.3-guided-step-by-step.md`.
