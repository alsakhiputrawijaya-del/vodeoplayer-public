...
function showHelp(kitDir) {
  ...
  L('              Args: --no-backup, --check-only, --cleanup-backups, --allow-downgrade, --dry-run')
  L('')
...
// CATATAN: diff SELALU pakai projectRoot/.lintasai (BUKAN $KitDir), beda dari
// doctor/status. Saat dipanggil langsung tanpa --project-root, ini menengok .lintasai di INDUK.
function invokeDiff(projectRoot) {
  const kitDir = path.join(projectRoot, NAMA_FOLDER_KIT)
  const manifestPath = path.join(kitDir, '.install-manifest.json')
  if (!fs.existsSync(manifestPath)) {
    // Awalan "WARNING:" (teks Indonesia) -> stderr.
...
function main(argv) {
  ...
  // "Inspecting").
  if (['doctor', 'version', 'status', 'diff'].includes(command)) {
  ...
  switch (command) {
    ...
    case 'check-update':
      // Panggil update-kit --check-only TANPA meneruskan ExtraArgs (read-only).
...
export { parseArgs, resolveDirs, getKitVersion, versionDisplay, invokeDoctor, invokeStatus, invokeDiff, showVersion, showHelp, main }
