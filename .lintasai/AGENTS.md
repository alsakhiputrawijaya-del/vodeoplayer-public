...
> Versi 7.0.0 · 2026-07-26 · Microkernel universal lintas-stack & lintas-harness.
> **Sumber tunggal aturan.** Berkas ini di akar project dibaca OTOMATIS tiap sesi oleh Codex, Kimi Code,
> Cursor (standar `AGENTS.md`) — dan oleh Claude Code lewat `CLAUDE.md` yang meng-`@import` berkas ini.
> Universal lintas-stack + lintas-harness: Claude Code · Kimi Code · Codex · Cursor.
> Konteks: mayoritas tugas = membuat/mengubah website + aplikasi.
> Kernel = 5 bagian: (1) Bahasa · (2) Anti-halusinasi · (3) Struktur-rapi · (4) Loop kerja · (5) AI tidak boleh merusak. Standar detail on-demand di skills/.
> Robot/hook = penguat opsional (Claude penuh · Kimi/Codex/Cursor tak ada) → SEMUA aturan jalan dari penalaran.
> Override khusus project (stack, opt-in mode) ada di `AGENTS.override.md` (Codex membacanya lebih dulu; milik project, tak ditimpa saat update).
> PATH: kit di-embed di `.lintasai/` → semua `skills/...`, `templates/...` & `registry.json` (di berkas ini & yang dirujuk dari dalam skill) ADA di `.lintasai/...` (working dir AI = root project).
> PRASYARAT POSISI: pastikan folder kit terjangkau dari working dir sesi — cek `.lintasai/` ada (di repo kit sendiri: `kit/`). Tidak ada → BERHENTI, beri tahu user buka ulang sesi AI dari folder tempat `.lintasai/` berada — di luar itu aturan & pengaman lintasAI TIDAK termuat, tanpa pesan error.
...
3.5  Konvensi rinci per-stack → ikut skill stack yang dibuka di langkah 4.3 (next-core · supabase-prisma ·
     react-patterns). Prinsip umum sudah di 3.1–3.4 & 3.6–3.9.
...
Alur: 4.1 PAHAMI → 4.2 DETEKSI → 4.3 SKILL → 4.4 BACA-KODE → 4.5 PUTUSKAN.
4.0  JALUR CEPAT — HANYA untuk yang JELAS bukan bikin/ubah kode (Q&A · baca-tunjuk file · typo) → jawab
     langsung, LEWATI loop (§1/§2/§5 TETAP berlaku · hemat token). Ragu / ada sentuhan kode → jalankan loop.
...
4.4  BACA-KODE (sebelum tulis/ubah) — baca kode asli target + cari pemanggilnya (siapa pakai fungsi/field/
     route ini). Kalau bug: perbaiki fungsi bersama SEKALI di akarnya, bukan tambal per-pemanggil (biar
     caller lain tak tertinggal rusak). Timbang hemat-kode: perlu ada? sudah ada / cukup 1 baris? → bangun
     minimum (kode terbaik = yang tak ditulis) — TAPI jangan pangkas validasi/keamanan/error-handling demi
     ringkas (§5). Sajikan rencana ringkas: ✅/❓ + sebut yang TIDAK dibangun.
4.5  PUTUSKAN (sebagai <divisi>-profesional dari 4.2 DETEKSI) — pakai teknik & standar dari SKILL (4.3),
     bukan dihafal kernel. Eksekusi 1 tugas · "selesai = terbukti": dijalankan/diuji — logika non-sepele
     tinggalkan 1 pemeriksaan jalan (assert/self-check/tes kecil, tanpa framework), one-liner sepele tak
     perlu · tinjauan multi-divisi tampil hanya bila ada temuan nyata: isinya dari divisi yang BERKASNYA
...
5.5  Aksi eksternal tak-bisa-dibatalkan (deploy/publish · pembayaran/refund nyata · kirim email/notif ke
     user nyata · hapus/nonaktif akun) → minta izin/konfirmasi dulu.
