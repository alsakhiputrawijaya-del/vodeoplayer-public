...
divisi: keamanan
pemicu: [permukaan ai, audit keamanan ai, mcp, ai-config, izin-tool, keamanan-skill, ai-chatbot, integrasi-ai]
...
- 💡 SARAN — **pemindai eksternal opsional (untuk OWNER kit, bukan langkah client):** `npx ecc-agentshield scan` (AgentShield, MIT © Affaan Mustafa) memindai permukaan yang sama secara deterministik + memberi nilai A–F. **JANGAN pakai `--fix`** (mengubah config = melanggar kontrak cuma-baca §1) dan **jangan dijadikan dependensi kit** — client wajib tetap bisa diperiksa tanpa jaringan. Nilainya = pembanding, bukan pengganti Langkah 0.
