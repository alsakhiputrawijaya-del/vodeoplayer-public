...
divisi: backend
pemicu: [realtime, real-time, websocket, socket.io, sse, server-sent-events, live-update, update-langsung, live-chat, typing-indicator, supabase-realtime, pusher, ably, presence-online, fitur-chat, chat-antar, pesan-langsung, tanpa-refresh, ojek-online, lacak-lokasi]
rawan_keamanan: true
menggantikan: []
---
...
> **Inti:** realtime = update terkirim langsung ke klien tanpa klien perlu refresh manual — tapi **hanya ke klien yang berhak** atas kanal itu (izin per-kanal di server). Kalau koneksi putus lalu nyambung lagi, klien wajib **menarik ulang state terkini** — bukan diam menampilkan data basi.

Realtime = **optimasi UX**, otoritas data tetap DB — koneksi hidup **≠** data pasti lengkap.
...
2. 🔒 **HASIL — Otorisasi per-kanal di SERVER saat langganan (subscribe), JANGAN percaya klien.** Klien bilang "aku mau kanal `chat:42`" → server WAJIB verifikasi sesi + apakah user ini anggota kanal 42 (cegah **nguping** kanal orang lain). Supabase Realtime: aktifkan **Realtime Authorization + RLS** (→ `skills/supabase-prisma/SKILL.md`), jangan andalkan filter di klien.
   ...
   - 🔒 **Sinyal project INI:** grep paket realtime (`socket.io`/`pusher`/`ably`/`@supabase/realtime-js`) → tahu model auth mana yang berlaku. **Default-bahaya:** kanal **broadcast/presence** TIDAK otomatis dijaga otorisasi (siapa tahu nama kanal bisa ikut nguping) → tetap WAJIB gate izin per-kanal di server.
...
5. 📐 **Skala & hosting:** koneksi persisten itu mahal & **serverless klasik (Vercel/Lambda) TAK cocok** untuk WebSocket berumur panjang (fungsi mati setelah beberapa detik). Untuk WS nyata → platform yang mendukung koneksi panjang (**Cloudflare Durable Objects**, server Node khusus) atau layanan terkelola. Skala horizontal butuh **pub/sub backplane** (mis. Redis) supaya pesan menyebar antar-instance (user di server A dapat event dari server B). Pilih hosting yang cocok → `skills/deploy/SKILL.md`.
...
- 💡 SARAN: pakai layanan/SDK teruji (Supabase Realtime, Ably, Pusher, `socket.io` + adapter Redis) daripada menulis server WS + backplane sendiri.
...
- [ ] Realtime **bukan otoritas data** (DB tetap otoritas; UI bisa re-fetch)?
...
- 📐 Rate-limit/anti-flood pesan masuk + anti badai-reconnect → `skills/rate-limiting/SKILL.md`; hosting yang mendukung koneksi persisten → `skills/deploy/SKILL.md`.
- 🗃️ LATAR — kerja berat di latar → `skills/background-job/SKILL.md`. Metrik koneksi → `templates/PRODUCTION_OBSERVABILITY.md`.
