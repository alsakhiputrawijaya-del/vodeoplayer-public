...
divisi: stack
pemicu: [vercel, railway, fly-io, render.yaml, deploy-platform, pilih-hosting, hosting-mana, biaya-hosting, platform-hosting]
...
1. 📐 **Env per-environment, bukan di repo.** Env var diatur terpisah untuk prod/preview/dev di dashboard platform; jangan pernah commit `.env` (🔒 §1).
2. 📐 **Build reproducible.** Lockfile + versi runtime dikunci & ikut di-commit (🔒 §1).
...
8. 📐 **Gerbang keamanan otomatis di pipeline (SAST · SCA · secret · container · DAST · SBOM).** Uji kode CEPAT & OTOMATIS tiap PR — jauh lebih murah daripada ketahuan setelah bocor. Tiap alat menutup lapisan berbeda; tak perlu semua sekaligus (lihat label WAJIB/DISARANKAN/OPSIONAL):
   ...
   - **Container scan (kalau pakai Docker).** `trivy image <img>` = pindai CVE base image + dependency; jalankan di CI sebelum push image (lengkapi Dockerfile — butir 12).
...
10. 📐 **Validasi env fail-fast saat startup (zod v4).** Validasi SEMUA env var sekali di startup pakai schema. Kalau ada yang salah/kosong → app **BERHENTI di titik itu** dengan pesan jelas, bukan `undefined` yang meledak jauh di dalam alur (susah dilacak). Contoh kode → §3.
...
> 🔒 Ingat pagar §2 butir 11 — route `/api/health/detailed` publik-default, batasi aksesnya (token/middleware/jaringan internal) sebelum tayang.
...
- 📐 **Batas token/biaya fitur AI/LLM** (pelengkap mitigasi Denial-of-Wallet §2 butir 5): batasi token per-permintaan + kuota harian + timeout + kunci LLM server-only.
