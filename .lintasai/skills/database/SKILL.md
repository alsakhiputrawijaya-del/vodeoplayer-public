...
divisi: database
pemicu: [migrasi, migration, skema, schema, tabel-baru, kolom-baru, rls, database, basis-data, index-database, constraint, simpan-data, data-pelanggan, data-pengguna, tabel-data, multi-cabang, kepisah-per, simpan-permanen, jangan-hilang, rekam-medis, data-pasien, data-kesehatan, data-nasabah]
rawan_keamanan: true
...
> `rawan_keamanan: true` → **sangat disarankan dibuka sebelum edit pertama** berkas skema/migrasi, karena salah di sini = **data hilang** atau **data penyewa lain bocor** secara **senyap** (tak kelihatan sampai sudah terlambat). Ditandai 🔒 di Petunjuk Rak.
>
> **Inti:** migrasi = perubahan struktur tabel — kalau serampangan, data lama bisa rusak/hilang. **RLS** (*Row Level Security*) = aturan siapa-boleh-baca-baris-mana dipasang di level database itu sendiri, bukan cuma "janji" di kode aplikasi untuk menyaring.

Butir **🔒 HASIL** menyangkut data yang tak boleh hilang/bocor = tak boleh gagal.
...
4. 📐 **Parameterized query / prepared statement WAJIB.** DILARANG merangkai SQL lewat string-concat input user (cegah **SQL injection**). Pakai query builder / ORM yang mem-parameter otomatis, atau placeholder (`$1`, `?`).
...
- 💡 SARAN: migrasi destruktif prod = **aksi merusak** → tampilkan rencana + rollback + minta konfirmasi verbatim. Jangan auto-jalankan `prisma migrate deploy` ke prod tanpa konfirmasi.
...
- [ ] Urutan kolom composite benar (kesetaraan `=` dulu, rentang belakangan) + tak ada **foreign key tanpa index** (audit query: `templates/STACK_GUIDE.md` §5.2)?
- [ ] Perubahan yang memutus klien pakai **expand-then-contract** (bukan rename/drop langsung)?

> **Verifikasi WAJIB cuma-baca**: membuktikan = baca skema/migrasi + `Grep` + menalar. Klaim yang cuma bisa diuji dengan mengubah data (mis. RLS benar-benar memblokir) → **minta owner jalankan di staging**, JANGAN jalankan `INSERT/UPDATE/DELETE/DROP` di DB live.
...
- [ ] **Gerbang Pra-Rilis LULUS** — migrasi "aman" = terbukti (dry-run + rollback teruji), bukan "kelihatannya jalan".
...
- 📐 **Cheat-sheet Postgres kelas-produksi** (pilih tipe index B-tree/composite/GIN/BRIN + urutan kolom kesetaraan-dulu-rentang-belakangan · partial/covering index · 3 query audit: FK tanpa index, query lambat, tabel kembung/bloat · tipe data benar `bigint`/`text`/`timestamptz`/`numeric` · guardrail `statement_timeout` per-role) → `templates/STACK_GUIDE.md` §5 (**baca saat kerja index/migrasi** — sudah terpasang di project client, tak menambah token per sesi).
...
- 🗃️ **LATAR:** Hak login DB junior/senior (role tiering: junior DML-only, DDL di senior) — susun SQL `GRANT`/`REVOKE` per schema sesuai kebutuhan project. Rak asal skill ini hanya di riwayat git (ADR-027).
