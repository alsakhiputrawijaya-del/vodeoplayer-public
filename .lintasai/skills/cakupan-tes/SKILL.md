...
4. 🔒 **HASIL — Jalankan + pastikan hijau.** Jangan klaim "teruji" tanpa benar-benar `run`. "0 masalah" dari perintah yang ERROR = palsu — pastikan alat tesnya benar-benar jalan sebelum percaya vonisnya.
...
- 📐 **Urutan bug-check mekanis** (tes+build otomatis DULU, merah = prioritas tertinggi) → `AGENTS.md` §4.5 "selesai = terbukti" (jangan diulang di sini).
- 📐 **Bug yang sudah LOLOS ke produksi → penjaga permanen** (human-gated, owner menyetujui) → contoh formatnya di `templates/BUKU_PELAJARAN.example.md`; salin jadi `docs/BUKU_PELAJARAN.md` di project saat bug pertama.
