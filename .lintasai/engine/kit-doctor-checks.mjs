...
export function checkAiTools(kitDir, projectRoot, emit) {
  ...
  // Codex & Kimi - baca AGENTS.md akar NATIF (ADR-032): kernel = acuan tunggal, tak perlu generator
  // maupun adaptor hook (adaptor hook Kimi DICABUT: opt-in yang tak pernah dinyalakan client).
  if (ada('AGENTS.md')) {
    emit('OK    Codex/Kimi     : baca kernel AGENTS.md akar secara native (acuan tunggal) · palang TIDAK berlaku')
    emit('        -> Codex & Kimi tak punya sistem hook. Yang sampai = teks aturannya (mutu aturan sama).')
    c.ok++
  } else {
