...
import { migrateOldAgentsMd } from './migrate-agents-md.mjs'
...
export function migrasiDanGerbangAgents(ctx) {
  ...
  // HANYA kalau AGENTS.override.md belum ada (lihat alasan 'override-sudah-ada' di engine/migrate-agents-md.mjs);
...
export function pasangDocsDanBerkasTim(ctx) {
  const { KitDir, projectRoot, manifestState } = ctx
  const { dryRun, skipTeamFiles } = ctx.args
  ...
  // 2 workflow GitHub (jalan di CI client, mustahil dijalankan dari dalam .lintasai/).
  if (!skipTeamFiles && !almostEmpty) {
    ...
    // Pengingat mode-tim: langkah lanjut yang TAK muncul di rangkuman akhir.
    console.log('')
    ...
    console.log('\nINFO: --skip-team-files aktif - lewati salin .github/.')
  }
  ctx.docsDir = docsDir
