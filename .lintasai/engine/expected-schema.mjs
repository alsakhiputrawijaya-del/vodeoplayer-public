...
//
// MASALAH yang dipecahkan: artefak klien (Lajur C - hidup di LUAR .lintasai/, disemai sekali saat
// pasang lalu jadi milik klien) membawa penanda format `schema_version`. Kit terus di-update (folder
// .lintasai/ diganti total), artefak klien TIDAK -> bisa timbul "version skew senyap" (sebagian
...
//      belum ikut (template contoh) -> perbarui yang ditunjuk.
...
// (wiring hook) belum ber-penanda;
// mereka dapat entri di sini SAAT diberi penanda (rencana §4 Mesin 2 / Langkah 3+). Berkas internal
// kit SENGAJA tak masuk peta: ia terganti utuh bersama kit tiap update,
// jadi tak mungkin ketinggalan versi (bukan Lajur C).

export const LINTAS_EXPECTED_SCHEMA = Object.freeze({
  // Kartu identitas project (akar project klien).
  'project.lintas.jsonc': 1,
  // Catatan-pasang: daftar resmi berkas terpasang (.lintasai/.install-manifest.json).
