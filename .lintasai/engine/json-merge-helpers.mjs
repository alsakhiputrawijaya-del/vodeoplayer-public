...
if (isMain) {
  ...
  try {
    if (sub === 'get' && process.argv[3]) {
      ...
      console.error('Pakai: node engine/json-merge-helpers.mjs get <settings.json>')
      console.error('   atau: node engine/json-merge-helpers.mjs merge <existing> <template> <output> [--dry-run]')
      process.exit(2)
    }
  } catch (e) {
