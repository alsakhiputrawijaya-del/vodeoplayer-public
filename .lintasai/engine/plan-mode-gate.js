 ...
 *      bisa diizinkan robot ini (dipakai-ulang, bukan ditulis-ulang -> satu tempat penilaian bahaya).
 ...
 *   - Berkas rahasia (.env, kunci, kredensial) TIDAK PERNAH diizinkan otomatis - walau cuma dibaca (§5.2).
 *
 * Versi  : 1.0.0
 * Tanggal: 2026-07-18
...
// Berkas rahasia: JANGAN pernah auto-izinkan, walau operasinya "cuma baca" (§5.2 - boundary keras).
...
const LINTASAI_BACA = new Set([
  'preflight', 'doctor', 'env-keys', 'unicode-check', 'plan-scout', 'version',
]);
...
function penggalAman(tok) {
  ...
  if (cmd === 'npx') {
    ...
    // (npx mengunduh + menjalankan paket dari internet - persis pola yang dilarang §5.4).
...
function decide(obj) {
  ...
  //     Dipakai-ulang (bukan ditulis-ulang) supaya penilaian bahaya tetap SATU tempat.
  try {
  ...
  if (ALAT_BACA.has(tool)) {
    // Berkas rahasia tak pernah auto-izin, walau cuma dibaca (§5.2).
