...
> Versi 5 · 2026-07-10 · auto-deployed oleh pemasang Node (v5: kit 100% Node - semua perintah lewat `npx lintasai <perintah>`)
...
Kit `lintasAI` di-update dengan **1 perintah** - entah lewat chat AI ("lintasAI v2.8.0 rilis, update") atau jalanin `npx lintasai@latest update` di terminal. **Tidak butuh akun GitHub, tidak butuh git** - versi terbaru diambil dari npm. Sistem otomatis menyiapkan versi baru di sebelah, memeriksanya, lalu menukar; versi lama disimpan sebagai cadangan; berkas kamu (`AGENTS.md`, `docs/`, kode app) **tidak disentuh**. Lalu AI **auto-pakai aturan baru** di sesi berikutnya.
...
Kamu update kalau: (a) ada perbaikan **keamanan penting**, (b) kamu **mau fitur baru**, atau (c) **sekali-sekali** biar tak terlalu ketinggalan.

### Kapan WAJIB vs OPSIONAL vs TIDAK perlu

| Situasi | Perlu update? |
|---|---|
| Update ber-label **`[SECURITY]`** (lihat §3 + §6) | ✅ **WAJIB - segera.** Ini satu-satunya yang "harus sekarang". |
...
Hasilnya: semua staff pakai versi kit yang sama, bukan versi yang beda-beda sendiri-sendiri - lebih tenang, lebih seragam, lebih aman.
...
| Tier | Label CHANGELOG | Contoh isi update | Aksi staff |
|------|-----------------|-------------------|------------|
| **1 - Silent** | (tanpa label) | Typo, perbaikan kalimat, fix link rusak | Cukup 1 perintah `npx lintasai@latest update`. Selesai. |
| **2 - AI auto-sync** | (tanpa label) | Aturan baru ditambahin, template baru, prompt baru | 1 perintah update. AI di sesi berikutnya auto-pakai aturan baru. Tidak perlu setup ulang. |
| **3 - [BREAKING]** | `[BREAKING]` | Struktur file/folder berubah, format CLAUDE.md ganti, nama file di-rename | Baca **"Migration Steps"** inline di CHANGELOG.md untuk versi itu. Biasanya 2-5 langkah. AI bisa bantu jalanin step-by-step. |
| **4 - [SCAN-REQUIRED]** | `[SCAN-REQUIRED]` | Aturan berubah sampai dokumen lamamu perlu ditinjau ulang | Minta AI di chat baru: "baca ulang struktur project lalu sesuaikan docs yang terdampak". AI pakai `git ls-files` + `Grep`, lalu usulkan perubahan. Kamu approve.

> 🔒 **Tanda `[SECURITY]` (TERPISAH dari 4 tingkat di atas):** kalau entry CHANGELOG ada `[SECURITY]`, itu perbaikan KEAMANAN — **pasang SEGERA** walau update-nya kecil. Tool update menampilkan peringatan merah.
...
### Mode 2 - Terminal/CLI (untuk power user / CI automation)
...
```
[OK] Fetched lintasAI v2.0.0 (was: v1.63.0)
...
- Sambungan internet AI lagi lemot, mau update lebih cepat.
...
> Owner kit fix typo kecil di salah satu berkas panduan. Kamu chat "update", AI bilang "cuma typo, aman 100%, update otomatis". Kamu lanjut kerja. Tidak ada yang berubah di workflow kamu.

**Skenario B: Tier 2 - Hari Rabu sore**
> Owner kit tambah pola kerja baru di aturan kit untuk handle task "code review". Kamu update. Sesi AI besok pagi, kamu tanya "review PR ini", AI auto-pakai pola baru tanpa kamu sadar.
...
> Owner kit ubah aturan dokumentasi: dulu cuma menyangkut `docs/`, sekarang juga `prisma/schema.prisma`. Kamu bilang ke AI di chat baru: "baca ulang struktur project lalu sesuaikan docs yang terdampak". AI meninjau, lalu propose update di beberapa file lama. Kamu approve.
...
### Cara 2 (advanced): Terminal/CLI

1. Buka terminal (PowerShell/CMD/bash - apa saja).
...
> 💡 Jadi staff **TIDAK** update sendiri-sendiri. Owner update + push (1x per repo); staff tinggal **tarik** (`git pull`). Hasilnya: semua orang di satu repo pakai versi kit **sama persis** — tidak ada beda-beda.
...
./.lintasai/templates/STACK_GUIDE.md              ← kamu edit kemarin
./.lintasai/templates/STACK_GUIDE.md.bak-20260604-1530      ← BACKUP versi kamu disini
./.lintasai/templates/STACK_GUIDE.md              ← versi baru kit di-install di sini
```

Setelah update, kamu bisa **diff manual** antara file baru vs `.bak` kamu, lalu merge perubahan yang masih relevan. AI bisa bantu kalau kamu chat "merge backup STACK_GUIDE.md kemarin ke versi baru".
...
|---|---|
| Berkas project (AGENTS.md, docs) ke-timpa/berubah, mau balikin **isinya** | **Cara 1** (`npx lintasai rollback`) |
| Kit **baru rusak** / AI jadi bingung / error import setelah update | **Cara 2** (kembalikan SELURUH folder kit) — `npx lintasai rollback` TIDAK membalikkan folder kit |

### Cara 1: `npx lintasai rollback` — balikin BERKAS PROJECT (bukan folder kit)
...
**Catatan:** jangan tertukar — Cara 1 cuma memulihkan isi berkas project, tidak memperbaiki folder kit yang rusak; Cara 2 dipakai kalau kit itu sendiri yang bermasalah.
...
**TIDAK.** Update cuma sentuh folder `./.lintasai/`. File proyek (mis. `docs/`, `prisma/`, `app/`) tidak ke-touch sama sekali. Kit dan proyek terpisah.
...
Auto. Tiap kali AI start sesi, dia baca `AGENTS.md` di akar project fresh. Jadi setelah update, sesi berikutnya = pakai aturan baru tanpa setting ulang.
...
Perlu. Owner kit terus improve aturan, prompt, dan contoh. Update = AI kamu makin pintar. Solo project lebih gampang lagi karena tidak perlu koordinasi tim.

---

## 11. Cross-reference

- **Istilah teknis** (mis. "sha256", "manifest", "atomic re-clone") → dijelaskan bahasa awam di tempat (`AGENTS.md` §1.1: istilah dipertahankan + gloss awam di kemunculan pertama).
- **Detail per versi** (apa berubah di v1.2.0 vs v1.3.0) → buka `./.lintasai/CHANGELOG.md`.
- **Aturan umum kerja AI-first** → `AGENTS.md` akar project (kernel, auto-dimuat tiap sesi; detail on-demand di `./.lintasai/skills/`).
- **Kalau bingung, chat AI**: "saya mau update lintasAI tapi tidak paham langkahnya" - AI guide step-by-step.

---

> Dokumen ini bagian dari kit `lintasAI`. Update guide ini sendiri ikut tier classification - kalau owner kit ubah strategy update, versi guide ini naik dengan `[BREAKING]` label.
