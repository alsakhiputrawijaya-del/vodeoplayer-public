# templates/SAFE_DATABASE_OPERATIONS.md - Ubah Struktur Database TANPA Bikin App Mati / Data Hilang

> Versi 1 · 2026-06-27 · Untuk Supabase/PostgreSQL (cocok juga Prisma). Gaya non-programmer + SQL konkret.
> Pelengkap, BUKAN pengganti: Row-Level Security = `skills/database/SKILL.md` · backup harian otomatis = `templates/github/workflows/backup-schemas.yml` · akses berjenjang junior/senior (DB role tiering) = minta AI susun SQL `GRANT`/`REVOKE` per schema (`skills/database/SKILL.md`). File ini fokus ke **2 hal yang belum dicakup**: (1) ubah struktur tanpa-downtime (*expand-then-contract*), (2) cara balik kalau migrasi gagal (*rollback runbook*).
...
- 👨‍🎓 **Junior:** perubahan skema (struktur tabel) = operasi paling rawan *breaking* + *data loss*. Aturan inti `skills/database/SKILL.md` sudah mewajibkan migrasi *idempotent* + *expand-then-contract* + *reversible* — file ini menjadikannya **langkah konkret siap-jalan**.
...
2. **Backup/snapshot DULU sebelum yang merusak** (DROP/hapus kolom/ubah tipe). Supabase: *Database → Backups* atau `pg_dump` schema target. *(Aksi merusak tetap konfirmasi verbatim — `AGENTS.md` §5.1.)*
...
Patuhi SAFE_DATABASE_OPERATIONS.md:
...
> AI akan menampilkan **rencana bertahap** (Alur Berpemandu) + minta persetujuan sebelum menyentuh data. Aksi merusak butuh konfirmasi ketik-frasa.
...
- 🙂 **Non-programmer:** ubah struktur data tanpa mematikan aplikasi — buat kolom/tabel baru dulu (kosong), pindahkan isi bertahap sambil app tetap jalan, baru buang yang lama setelah kosong. Bukan bongkar struktur lama langsung (data berantakan + pengguna terganggu).
...
- [ ] RLS tetap utuh setelah migrasi (kalau tabel ber-RLS — cek `skills/database/SKILL.md`).

---

## 🙂 Untuk non-programmer (ringkas)

Berkas ini = **panduan mengubah struktur database tanpa mematikan aplikasi**. Aturannya: buat struktur baru dulu di samping yang lama (jangan bongkar yang lama langsung), pindahkan isi bertahap, backup sebelum membuang apa pun, dan siapkan rollback (cara balik ke versi sebelumnya) sebelum mulai. Terapkan bertahap — yang baru masuk dulu, yang lama dibuang belakangan setelah aman.
