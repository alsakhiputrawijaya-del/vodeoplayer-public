...
2. Tinjau hasil simulasi → kalau cocok, terapkan: `<perintah 1-baris>`
   (migrator WAJIB idempoten + membuat cadangan ber-cap-waktu)
3. Verifikasi: `npx lintasai doctor`.

**Cara balik (rollback) kalau ada masalah:** <1 baris, mis. pulihkan dari cadangan .backup-<ts>>
```

Rujukan maintainer: `docs/RESEP_PERUBAHAN.md`. `engine/expected-schema.mjs` menyimpan peta
`schema_version` per-artefak (kartu project + catatan-pasang) yang dipakai pemeriksa `doctor`/`preflight`.
...
`--no-backup`) juga bisa dipakai — lihat bagian **Rollback** di `templates/UPDATE_GUIDE.md`.
...
| Dokumen install `MULAI_DI_SINI.md` + `MCP_SETUP.md` | Install cukup `npm create lintasai` (README); keamanan DB: `skills/database/SKILL.md` + `docs/SAFE_DATABASE_OPERATIONS.md` |
